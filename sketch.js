...
function preload()
{
	...
	default_bill = loadAnimation("images/19.png");
...
function draw() {
  ...
  if(keyDown("DOWN")){
	bill.changeAnimation("default1",billbackImage);
	bill.y = bill.y+6;
}

if(keyDown("LEFT")){
	bill.changeAnimation("default1",billleft_Image);
	bill.x = bill.x-6;
}

if(keyDown("RIGHT")){
	bill.changeAnimation("default3",billright_Image);
	bill.x = bill.x+6;
}
